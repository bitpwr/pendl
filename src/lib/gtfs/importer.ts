...
export async function importGtfsToDatabase(data: ImportData): Promise<void> {
  ...
  try {
    ...
    await client.query('TRUNCATE agencies, stops, routes, trips, stop_times, calendar, calendar_dates, shapes CASCADE');
    ...
    await client.query('REFRESH MATERIALIZED VIEW CONCURRENTLY stop_route_types');
